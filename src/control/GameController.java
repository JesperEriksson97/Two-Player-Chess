...
public class GameController implements IGameController {
	...
	public void squareClicked(Position clickedPosition) {
		...
		IChessPiece clickedPiece = chessBoard.getPieceAtPosition(clickedPosition);
	...
	private void attemptPiecePlacement(Position clickedPosition) {
		...
		gcState.currentPlayerToMove = (gcState.getCurrentPlayerToMove() == IChessPiece.Colour.WHITE) ? IChessPiece.Colour.BLACK : IChessPiece.Colour.WHITE;
	...
	private void attemptToPickUpPiece(IChessPiece clickedPiece) {
	...
	public List<Position> getAllowedMovesForPiece(IChessPiece clickedPiece) {
		List<Position> possibleMoves = new ArrayList<Position>();
		List<List<Position>> initialPossibleMoves = clickedPiece.deriveAllMoves();
		possibleMoves = cullIllegalMoves(initialPossibleMoves, clickedPiece);
		addSpecialCases(possibleMoves, clickedPiece);
		assert !possibleMoves.contains(null);
		return possibleMoves;
	}
	 ...
	 */
	private List<Position> getCheckedLine(IChessPiece clickedPiece) {
		...
		if (line == null)
			return null;
		for (Position p : line) {
			IChessPiece threateningPiece = chessBoard.getPieceAtPosition(p);
	 ...
	 * to the line.
	 */
	private List<Position> calculateSharedOpenLine(King king, IChessPiece clickedPiece,
			MutableBoolean isDiagonalLine) {
		int xDiff = clickedPiece.getPosition().getXCoord() - king.getPosition().getXCoord();
		int yDiff = clickedPiece.getPosition().getYCoord() - king.getPosition().getYCoord();
		...
		while (true) {
			...
			if (chessBoard.getPieceAtPosition(positionToAdd) != null) {
				...
				if (chessBoard.getPieceAtPosition(positionToAdd) != clickedPiece) {
		...
		if (clickedPiece instanceof Pawn) {
			...
			for (int xDisp = -1; xDisp < 2; xDisp += 2) {
				Position forwardDiagonal = Position.createPosition(clickedPiecePosition.getXCoord() + xDisp,
						clickedPiecePosition.getYCoord() + forwardMove);
				IChessPiece potentialVictim = chessBoard.getPieceAtPosition(forwardDiagonal);
	 ...
	 */
	private void addSpecialCases(List<Position> possibleMoves, IChessPiece clickedPiece) {
	...
	private boolean positionIsChecked(Position position) {
		...
		for (Position knightPosition : knightAttackPositions) {
			IChessPiece possibleKnight = chessBoard.getPieceAtPosition(knightPosition);
		...
		for (int xDisp = -1; xDisp < 2; xDisp++) {
			for (int yDisp = -1; yDisp < 2; yDisp++) {
				...
				for (;; x += xDisp, y += yDisp) {
					...
					IChessPiece threateningPiece = chessBoard.getPieceAtPosition(nextPosition);
	...
	private boolean isThreateningPawn(
		...
		if (pawn.getColour() == IChessPiece.Colour.BLACK && yDiff == 1
				|| pawn.getColour() == IChessPiece.Colour.WHITE && yDiff == -1) {
			return true;
		}
		return false;
	}
	...
	private void performCastling(Position clickedPosition) {
		...
		IChessPiece rookToCastle = chessBoard.getPieceAtPosition(rookPosition);
	...
	private void checkForStalemate() {
		...
		if (stalemateOption == StalemateOption.MANDATORY_PLAYER_CANT_MOVE
			...
			endGame.informThatPlayerMayDeclareStalemate(stalemateOption,
					gcState.getCurrentPlayerToMove() == Colour.WHITE ? IChessPiece.Colour.BLACK : Colour.WHITE);
	}
	...
	private boolean isCheckmate() {
		...
		List<IChessPiece> currentPlayersPieces = chessBoard.getPlayersPieces(gcState.currentPlayerToMove);
		for (IChessPiece chessPiece : currentPlayersPieces) {
		...
		if (gcState.checkBlockingMoves.size() == 1 && gcState.checkBlockingMoves.get(0).equals(gcState.enPassantPosition)) {
			for (int i = -1; i < 2; i += 2) {
				...
				IChessPiece friendlyPawn = chessBoard.getPieceAtPosition(potentialFriendlyPawnPosition);
	...
	public ChessBoardMoment captureCurrentMoment() {
		Map<Position, IChessPiece> chessPieces = chessBoard.getChessPiecesClone();
	...
	private CastlingOpportunities constructCastlingOpportunities() {
		...
		for (int yCoord = 1; yCoord <= 8; yCoord += 7) {
			...
			IChessPiece supposedKing = chessBoard.getPieceAtPosition(reqKingPosition);
	...
	private boolean canCastleBetweenPositions(Position kingPosition, Position rookPosition, int direction) {
		IChessPiece potentialRook = chessBoard.getPieceAtPosition(rookPosition);
	...
	private CastlingPiecesMovementTracker constructCastlingPiecesMovementTracker() {
		...
		for (int yCoord = 1; yCoord <= 8; yCoord += 7) {
			for (int xCoord = 1; xCoord != 12; xCoord += 4, counter++) {
				...
				IChessPiece pieceAtPosition = chessBoard.getPieceAtPosition(position);
